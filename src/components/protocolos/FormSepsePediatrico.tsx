...
export const FormSepsePediatrico = ({ onBack }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Atendimento de sepse pediátrico registrado!');
      onBack();
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-bold">Protocolo de Sepse Pediátrico</h2>
          <p className="text-sm text-muted-foreground">Registrar novo atendimento pediátrico • Meta ATB: ≤ {meta} min</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card><CardHeader><CardTitle className="text-base">Identificação</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div><Label>Idade</Label><Input type="number" value={age} onChange={e => setAge(e.target.value)} placeholder="Em meses ou anos" /></div>
        ...
        <Card><CardHeader><CardTitle className="text-base">Abertura do Protocolo</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div><Label>Setor</Label><Input value={protocolSector} onChange={e => setProtocolSector(e.target.value)} /></div>
            <div className="flex items-center gap-2 md:col-span-3"><Checkbox checked={sepseSuspeita} onCheckedChange={v => setSepseSuspeita(!!v)} id="susp-p" /><label htmlFor="susp-p" className="text-sm font-medium cursor-pointer">Suspeita de Sepse</label></div>
            {sepseSuspeita && (<><div><Label>Motivo</Label><Input value={sepseMotivo} onChange={e => setSepseMotivo(e.target.value)} /></div><div><Label>Horário</Label><Input type="datetime-local" value={sepseHorario} onChange={e => setSepseHorario(e.target.value)} /></div></>)}
          </CardContent>
        </Card>

        <Card><CardHeader><CardTitle className="text-base">Critérios SIRS Pediátricos</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {[
              { k: 'temp_alta', l: 'Temperatura > 38,5°C' }, { k: 'temp_baixa', l: 'Temperatura < 36°C' },
              { k: 'fc_alta', l: 'FC elevada p/ idade' }, { k: 'fr_alta', l: 'FR elevada p/ idade' },
              { k: 'leucocitose', l: 'Leucocitose' }, { k: 'leucopenia', l: 'Leucopenia' },
              { k: 'celulas_jovens', l: '> 10% cél. jovens' }, { k: 'plaquetas', l: 'Plaquetopenia' },
              { k: 'lactato', l: 'Lactato elevado' }, { k: 'pa_baixa', l: 'Hipotensão p/ idade' },
              { k: 'sato2_baixa', l: 'SpO₂ < 90%' }, { k: 'consciencia', l: 'Alteração consciência' },
            ].map(c => (<div key={c.k} className="flex items-center gap-2"><Checkbox checked={sirs[c.k] || false} onCheckedChange={() => toggleSirs(c.k)} id={`sp-${c.k}`} /><label htmlFor={`sp-${c.k}`} className="text-sm cursor-pointer">{c.l}</label></div>))}
          </CardContent>
        </Card>

        <Card><CardHeader><CardTitle className="text-base">Sinais Vitais</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div><Label>PA</Label><Input value={vitalPa} onChange={e => setVitalPa(e.target.value)} /></div>
            ...
            <div><Label>Temp</Label><Input type="number" step="0.1" value={vitalTemp} onChange={e => setVitalTemp(e.target.value)} /></div>
          </CardContent>
        </Card>

        <Card><CardHeader><CardTitle className="text-base">Foco Infeccioso</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {[{ k: 'pulmonar', l: 'Pulmonar' }, { k: 'urinario', l: 'Urinário' }, { k: 'abdominal', l: 'Abdominal' }, { k: 'pele', l: 'Pele/Partes Moles' }, { k: 'cateter', l: 'Cateter' }, { k: 'sem_foco', l: 'Sem foco definido' }].map(c => (
              <div key={c.k} className="flex items-center gap-2"><Checkbox checked={foco[c.k] || false} onCheckedChange={() => toggleFoco(c.k)} id={`fp-${c.k}`} /><label htmlFor={`fp-${c.k}`} className="text-sm cursor-pointer">{c.l}</label></div>
            ))}
          </CardContent>
        </Card>
        ...
        <Card><CardHeader><CardTitle className="text-base">Kit Sepse e Laboratório</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center gap-2"><Checkbox checked={kitSepse} onCheckedChange={v => setKitSepse(!!v)} id="kit-p" /><label htmlFor="kit-p" className="text-sm font-medium cursor-pointer">Kit Sepse Coletado</label></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><Label>Lab - Horário Chamado</Label><Input type="datetime-local" value={labChamado} onChange={e => setLabChamado(e.target.value)} /></div>
              <div><Label>Lab - Horário Coleta</Label><Input type="datetime-local" value={labColeta} onChange={e => setLabColeta(e.target.value)} /></div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card><CardHeader><CardTitle className="text-base">Antibioticoterapia</CardTitle></CardHeader>
          <CardContent className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div><Label>ATB</Label><Input value={atb1Nome} onChange={e => setAtb1Nome(e.target.value)} /></div>
        ...
        <Card><CardHeader><CardTitle className="text-base">Choque Séptico</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center gap-2"><Checkbox checked={choqueSeptico} onCheckedChange={v => setChoqueSeptico(!!v)} id="choque-p" /><label htmlFor="choque-p" className="text-sm font-medium cursor-pointer">Choque Séptico</label></div>
            {choqueSeptico && (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><Label>Reposição Volêmica</Label><Input type="datetime-local" value={choqueReposicao} onChange={e => setChoqueReposicao(e.target.value)} /></div>
              <div><Label>Vasopressor</Label><Input type="datetime-local" value={choqueVasopressor} onChange={e => setChoqueVasopressor(e.target.value)} /></div>
            </div>)}
          </CardContent>
        </Card>

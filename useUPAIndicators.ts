...
export const useUPAIndicators = () => {
  ...
  const filteredIndicators = useMemo(() => {
    return indicators.filter((ind: IndicatorData) => ind.mes === selectedMonth && ind.ano === selectedYear);
  ...
  const saveIndicator = async (indicator: Partial<IndicatorData> & { mes: string; ano: number; categoria: string; indicador: string }) => {
    try {
      const existing = indicators.find(
        (i: IndicatorData) => i.mes === indicator.mes && i.ano === indicator.ano && i.categoria === indicator.categoria && i.indicador === indicator.indicador
      );
      if (existing) {
      ...
      queryClient.invalidateQueries({ queryKey: ['upa_indicators'] });
  ...
  const createActionPlan = async (plan: Omit<ActionPlan, 'id' | 'created_at' | 'updated_at'>) => {
    try {
      ...
      if (error) throw error;
      queryClient.invalidateQueries({ queryKey: ['upa_action_plans'] });
  ...
  const calculateStats = useMemo(() => {
    ...
    const totalAtendimentos = filteredIndicators.find((i: IndicatorData) => i.indicador === 'Nº Total de Atendimentos')?.valor_numero ?? 0;
    const taxaMortalidade = filteredIndicators.find((i: IndicatorData) => i.indicador === 'Taxa de Mortalidade')?.valor_numero ?? 0;
    const colaboradores = filteredIndicators.find((i: IndicatorData) => i.indicador === 'Número de Colaboradores')?.valor_numero ?? 0;
  ...
  return {
    ...
    refetch: () => queryClient.invalidateQueries({ queryKey: ['upa_indicators'] }),
  };
};

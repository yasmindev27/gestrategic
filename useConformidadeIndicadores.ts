...
async function fetchConformidade() {
    try {
      ...
      return {
        ...
        indicadoresPorModulo: [
          { modulo: "Segurança do Paciente (ONA)", indicadores: indicadoresSegurancaPaciente, score_geral: onaScore },
          { modulo: "Gestão da Qualidade (ISO 9001)", indicadores: indicadoresGestaoQualidade, score_geral: isoScore },
          { modulo: "Infraestrutura", indicadores: indicadoresInfraestrutura, score_geral: Math.round((taxaRondas / 100) * 100) },
          { modulo: "RH / Segurança do Trabalho", indicadores: indicadoresRH, score_geral: Math.round(((taxaEPIsValidos + taxaVacinacao + taxaEscalas) / 3)) },
        ],
      };
    } catch (error) {
...
export const useConformidadeIndicadores = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["conformidade_indicadores"],
    queryFn: fetchConformidade,
  });

  const defaultConformidade: ConformidadeGeral = { ona_score: 0, iso_score: 0, qmentum_score: 0, total_indicadores: 0, conformes: 0, alertas: 0, nao_conformes: 0 };

  return {
    isLoading,
    conformidadeGeral: data?.conformidadeGeral ?? defaultConformidade,
    indicadoresPorModulo: data?.indicadoresPorModulo ?? [],
    recarregar: () => queryClient.invalidateQueries({ queryKey: ["conformidade_indicadores"] }),
  };
};
